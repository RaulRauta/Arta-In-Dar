  ...
  return <section className="sculpture-gallery" id="galerie-sculpturi">
    <div className="shell">
      ...
      <div className="sculpture-gallery__grid">
        {sculptures.map((sculpture,index)=><motion.button type="button" initial={false} animate={{ opacity: 1, y: 0, rotate: 0 }} transition={{ duration: .62, delay: (index % 6) * .055, ease: [0.22,1,0.36,1] }} whileHover={{ y: -8, rotate: index % 2 ? .25 : -.25 }} whileTap={{ scale: .985 }} key={`${sculpture.title}-${sculpture.artist}`} className="sculpture-card sculpture-card--button" onClick={() => setActiveIndex(index)} aria-label={`Deschide fișa lucrării ${sculpture.title}`}>
